...
// Simple 8x16 bitmap font data for classic text mode appearance
...
public:
    ...
    static const uint8_t* getCharData(char ch);
    
private:
    // Font data for ASCII characters 32-127
    static const uint8_t fontData[96][16];
};

// Font data - simplified 8x16 font
const uint8_t BitmapFont::fontData[96][16] = {
    // Character 32 (Space)
...
const uint8_t* BitmapFont::getCharData(char ch) {
    int index = static_cast<int>(ch) - 32;
    if (index < 0 || index >= 96) {
        // Return space character for out-of-range
        return fontData[0];
    }
